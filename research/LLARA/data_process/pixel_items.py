...
def item_stats(item_texts): 
    ...
    for iid in item_texts.keys(): 
        ...
        if value['tag'] not in same_tag: 
            same_tag[value['tag']] = 0
        same_tag[value['tag']] += 1  
        ...
        counts += 1
        # unique item rep 
        combinations.add(f"title: {value['title']}, tag: {value['tag']}, description: {value['description']}")
