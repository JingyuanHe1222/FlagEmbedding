...
import nltk
nltk.data.path.append("/data/user_data/jingyuah/hub/nltk_data")
nltk.download('stopwords')

from nltk.corpus import stopwords


class TrainItemEBAEDatasetForEmbedding(Dataset):
    def __init__(
            self,
            dataset,
        ...
        self.prefix = '"'
        self.suffix = ['", “compress the following sentence into embeddings: <unk>']
    ...
    def __getitem__(self, item):
        ...
        input_ids = self.tokenizer(inp,
                                   truncation=True,
                                   max_length=self.max_length - len(prefix_ids) - len(suffix_ids_summarize),
                                   padding=False,
                                   return_tensors=None,
                                   add_special_tokens=False)
        result = dict()
        result['input_ids'] = prefix_ids + input_ids['input_ids'] + suffix_ids_summarize
        result['attention_mask'] = [1] * len(result['input_ids'])
        result['labels'] = [-100] * len(prefix_ids) + result['input_ids'][
            len(prefix_ids) : len(result['input_ids']) - len(suffix_ids_summarize)] + [
            -100] * (len(suffix_ids_summarize))
        ...
        return result, oup_summarize


@dataclass
class ItemEBAEEmbedCollator(DataCollatorForSeq2Seq):
    """
...
class TrainDatasetForEmbedding(Dataset):
    def __init__(
        ...
        self.suffix = ['", compress the above item sequence into embeddings: <unk>',
                       '", predict the next item embedding: <unk>']
        print("suffix: ", self.suffix)
        self.prefix_ids = self.tokenizer(self.prefix, truncation=True, max_length=self.max_length, return_tensors=None)['input_ids']
        self.suffix_ids = self.tokenizer(self.suffix, truncation=True, max_length=self.max_length, return_tensors=None, add_special_tokens=False)['input_ids']
