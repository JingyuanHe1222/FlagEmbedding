...
def get_passage_inputs(passages, tokenizer, max_length=512):
    prefix = '"'
    suffix = '", compress the above item sequence into embeddings: <unk>'
...
def custom_collator(batch):
    ...
    item_list = []
    for input in batch: 
        trim_query = "title:" + input['input'].split(";  title:")[-1] 
        query_list.append(trim_query)
        item_list.append(input['output_predict'])
...
model_path = '/data/user_data/jingyuah/LLARA/checkpoints/pixel_200k_item_token_EBAE_logs/checkpoint-1400'
# model_path = '/data/user_data/jingyuah/LLARA/checkpoints/pixel_200k_item_token_EBAE_logs/checkpoint-1400/Pixel200K_SFT_fixed_tr0.2_lr_1e-6/checkpoint-1500'
...
eval_dataset = eval_dataset.train_test_split(test_size=0.3, seed=42)['test']
...
with torch.no_grad():
    ...
    for idx, batch in tqdm(enumerate(evalset), total=len(evalset)): 
        ...
        # treat everything as passage (item) embed 
        query_input = get_passage_inputs(batch['query'], tokenizer, 256)
        passage_input = get_passage_inputs(batch['item'], tokenizer, 256)
        # encode query item
        ...
        passage_embedding = passage_outputs.hidden_states[-1][:, -num_passage_token:, :].detach().cpu().squeeze()
...
with open('/data/user_data/jingyuah/LLARA/checkpoints/pixel_200k_item_token_EBAE_logs/checkpoint-1400/Pixel200K_SFT_fixed_tr0.2_lr_1e-6/checkpoint-1500/trained_eval_3000_embed.pkl', 'wb') as f:
    pickle.dump((query_embeddings, passage_embeddings), f)


# # # load directly
# with open('/data/user_data/jingyuah/LLARA/checkpoints/pixel_200k_item_token_EBAE_logs/checkpoint-1400/trained_eval_3000_embed.pkl', 'rb') as f:
#     query_embeddings, passage_embeddings = pickle.load(f)

passage_embeddings = passage_embeddings.squeeze()
